...
**לפני (הגרסה המקורית):** גרסה 0.0.10 — בקרת זמן שימוש במחשב: מסך נעילה עם תאריך עברי, חסימת מקשים (Windows/Alt+Tab/Ctrl+Alt+Del...), ניטור תוכנות, מדפסות עם חיוב אוטומטי, שוברים.

**מה שופר בגרסה הנוכחית:**
- הקוד עבר ריפקטורינג ושכתוב מלא (20+ מודולים: lock_screen, admin_panel, registry_manager, print_monitor...)
- נוסף תיעוד טכני (`docs/`)
- נוסף קובץ `Shomer-HaPetach.spec` לבניית EXE חוזרת
- נבנה ופורסם EXE מוכן (Shomer-HaPetach.exe) ב-`dist/` — הורדה והרצה ללא Python
- נוסף רישיון + README מפורט בעברית עם באנר, היסטוריית גרסאות וקישור הורדה
